...
def print_main_menu():
    ...
    print(Fore.LIGHTYELLOW_EX + "6.", Fore.BLUE + "Lab 7 - Distributed Polynomial (MPI)")
    print(Fore.LIGHTYELLOW_EX + "7.", Fore.BLUE + "Lab 8 - Distributed Shared Memory (DSM)")
    print(Fore.LIGHTYELLOW_EX + "8.", Fore.RED + "Exit")
...
def print_lab7_menu():
    print(Fore.CYAN + "=" * 40)
    print(Fore.CYAN + "Lab 7 - Distributed Polynomial Multiplication (MPI)")
    print(Fore.CYAN + "=" * 40)
    print(Fore.LIGHTYELLOW_EX + "1.", Fore.BLUE + "Run MPI Polynomial Performance Comparison")
    print(Fore.LIGHTYELLOW_EX + "2.", Fore.MAGENTA + "Bonus: Run MPI Big Number Performance Comparison")
    print(Fore.LIGHTYELLOW_EX + "3.", Fore.YELLOW + "Back to Main Menu")

def print_lab8_menu():
    print(Fore.CYAN + "=" * 40)
    print(Fore.CYAN + "Lab 8 - Distributed Shared Memory (DSM)")
    print(Fore.CYAN + "=" * 40)
    print(Fore.LIGHTYELLOW_EX + "1.", Fore.BLUE + "Run DSM Demo")
    print(Fore.LIGHTYELLOW_EX + "2.", Fore.YELLOW + "Back to Main Menu")
...
def lab7_menu():
    """Lab 7 menu - runs external MPI scripts using mpiexec."""
    while True:
        print_lab7_menu()
        choice = input(Fore.CYAN + "Enter your choice (1-3): " + Fore.RESET)

        if choice == "1":
            print(Fore.GREEN + "Running MPI polynomial performance comparison...")
            _run_mpi_script("polynomial_mpi.py", "polynomial")
        elif choice == "2":
            print(Fore.GREEN + "Running MPI big number performance comparison...")
            _run_mpi_script("polynomial_mpi.py", "big_numbers")
        elif choice == "3":
            break
        else:
            print(Fore.RED + "Invalid choice. Please try again.")
...
def lab8_menu():
    """Lab 8 menu - runs DSM demo using MPI."""
    while True:
        print_lab8_menu()
        choice = input(Fore.CYAN + "Enter your choice (1-2): " + Fore.RESET)

        if choice == "1":
            print(Fore.GREEN + "Running DSM demo...")
            print(Fore.CYAN + "This will run with 3 MPI processes.")
            _run_mpi_script("dsm_demo.py", None, num_processes=3)
        elif choice == "2":
            break
        else:
            print(Fore.RED + "Invalid choice. Please try again.")
...
def _run_mpi_script(script_name: str, mode: str = None, num_processes: int = 4):
    """
    Helper to run an MPI-based Python script via mpiexec.

    We pass an environment variable LAB7_MODE so that the script can
    decide whether to run polynomial or big-number tests.
    """
    # Detect mpiexec in PATH
    mpiexec_path = shutil.which("mpiexec")
    if mpiexec_path is None:
        print(Fore.RED + "Error: mpiexec not found in PATH.")
        print(Fore.YELLOW + "Please install MPI (e.g., MS-MPI / OpenMPI) and mpi4py.")
        return
    ...
    try:
        result = subprocess.run(
            [mpiexec_path, "-n", str(num_processes), "python", script_name],
            capture_output=True,
            text=True,
            cwd=os.getcwd(),
            env=env,
        ...
        print(Fore.RED + f"Error running MPI script: {e}")

def main():
    while True:
        print_main_menu()
        choice = input(Fore.CYAN + "Enter your choice (1-8): " + Fore.RESET)
