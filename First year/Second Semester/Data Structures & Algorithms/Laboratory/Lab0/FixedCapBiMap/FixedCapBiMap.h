...
class FixedCapBiMap {
    ...
    //removes a key k with all values that belong to it. Returns a ValuePair containing the remove values.
    // - if k had 2 associated values, these are in the ValuePair
    // - if k had 1 value, that values is the first in the ValuePair
    // the second value will be NULL_TValue
    // - if k had no associated Value => both entries of returned result are NULL_TValue
