...
void *f(void *a)
{
	...
	{
		if((*d->len_U) == 0)
			printf("First thread to add to U \n");
		d->U[(*d->len_U)] = d->number;
		(*d->len_U)++;
	}
	else
	{
		if((*d->len_L) == 0)
			printf("First thread to add to L \n");
		d->L[(*d->len_L)] = d->number;
		(*d->len_L)++;
	}
	(*d->S) += d->number;
...
int main(int argc, char** argv)
{
	...
	for(int i = 0; i < N; i++)
	{
		printf("Enter the number for thread %d: ", i + 1);
		scanf("%d", &numbers[i]);
		d[i].L = L;
		d[i].len_L = &len_L;
		d[i].U = U;
