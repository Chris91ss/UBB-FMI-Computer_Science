...
def print_solution(all_bases: list):
    with open(file_name + 'output.out', 'w') as test_out:
        test_out.write(f"The number of {k}-dimensional subspaces of the vector space (Z2)^{n} "
                       f"over Z2 is {len(all_bases)}\n")

        test_out.write("\nA basis of each such subspace is :\n\n")
        for basis in all_bases:
            test_out.write(f"{basis}\n\n")
...
def get_integers() -> tuple[int, int]:
    try:
        with open(file_name + 'input.in', 'r') as test_in:
            lines = test_in.readlines()
            integer_1 = int(lines[0])
            integer_2 = int(lines[1])

            if integer_1 not in range(1, 7) or integer_2 not in range(1, 7) or integer_1 > integer_2:
                raise ValueError
