...
def print_matrix(matrix):
    ...
    for i in range(n):
        for j in range(n):
            test_out.write(f"a{str(matrix[i][j])} ")
        test_out.write("\n")
    test_out.write("\n")
...
if __name__ == '__main__':
    test_in = open("tests/test_5_input.in", 'r')
    test_out = open("tests/test_5_output.out", 'w')

    n = int(test_in.readline())
    test_in.close()

    test_out.write("Let set A = {")  # this displays the set A
    ...
    test_out.write("The operation tables of the associative operations on set A are given by the matrices : ")
    test_out.write("\n")
    test_out.write("\n")
    find_tables(0)

    test_out.write(f"The number of associative operations on set A is : {number_of_associative_operations}")
    test_out.close()
